...
class Register:
    def __init__(self, root):
        ...
        question = tk.Label(frame1, text="Security Question", font=("times new roman", 15, "bold"), bg="#161B21",
                            fg="#F4A950")
        ...
        cpasswd = tk.Label(frame1, text="Confirm Password", font=("times new roman", 15, "bold"), bg="#161B21",
                           fg="#F4A950")
        ...
        btn_register = tk.Button(frame1, text="Register Now", font=("times new roman", 13), bg="#161B21", fg="#F4A950",
            ...
            try:
                ...
                if row != None:
                    messagebox.showerror("Error",
                                         "User already exists with same email address,Register with another Email address",
                                         parent=self.root)
                else:
                    cur.execute(
                        "insert into regfields (f_name,l_name,contact,email,question,answer,password) values(%s,%s,%s,%s,%s,%s,%s)",
                        (self.txt_fname.get(), self.txt_lname.get(), self.txt_contact.get(), self.txt_email.get(),
                         self.cmb_quest.get(), self.txt_answer.get(), self.txt_passwd.get()))
                    con.commit()
                    con.close()
